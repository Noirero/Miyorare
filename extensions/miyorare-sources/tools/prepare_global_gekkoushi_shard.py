...
'''
    ...
    # The upstream parser puts the exact-match '    # Details can render a usable Read/Continue action without waiting for a second network round-trip.
...
if __name__ == "__main__":
 ...
 after the closing quote:
    # tag:"ai generated"$ / -tag:"ai generated"$ / language:"english"$.
    # E-Hentai's search grammar requires the '    # Details can render a usable Read/Continue action without waiting for a second network round-trip.
    # getDetails() later enriches the same chapter id with upload date/language metadata.
    old_list_tail = '''                authors = setOfNotNull(author),
                source = source,
            )
...
if __name__ == "__main__":
    main()
 inside the quoted value instead:
    # tag:"ai generated$" / -tag:"ai generated$" / language:"english$".
    # With the old form a tag could look selected in Miyorare while the website search did not
    # actually constrain the result set as intended.
    old_exact_query = '''            joiner.add("tag:\\\"")
            joiner.append(tag.key)
            joiner.append("\\\"$")
'''
    new_exact_query = '''            joiner.add("tag:\\\"")
            joiner.append(tag.key)
            joiner.append("$\\\"")
'''
    if text.count(old_exact_query) != 1:
        fail("Pinned ExHentai parser changed: include-tag exact query block not found exactly once")
    text = text.replace(old_exact_query, new_exact_query, 1)

    old_exact_exclude = '''            joiner.add("-tag:\\\"")
            joiner.append(tag.key)
            joiner.append("\\\"$")
'''
    new_exact_exclude = '''            joiner.add("-tag:\\\"")
            joiner.append(tag.key)
            joiner.append("$\\\"")
'''
    if text.count(old_exact_exclude) != 1:
        fail("Pinned ExHentai parser changed: exclude-tag exact query block not found exactly once")
    text = text.replace(old_exact_exclude, new_exact_exclude, 1)

    old_exact_locale = '''            joiner.add("language:\\\"")
            joiner.append(lc.toLanguagePath())
            joiner.append("\\\"$")
'''
    new_exact_locale = '''            joiner.add("language:\\\"")
            joiner.append(lc.toLanguagePath())
            joiner.append("$\\\"")
'''
    if text.count(old_exact_locale) != 1:
        fail("Pinned ExHentai parser changed: locale exact query block not found exactly once")
    text = text.replace(old_exact_locale, new_exact_locale, 1)

    old_exact_artist = '''            joiner.add("artist:\\\"")
            joiner.append(a)
            joiner.append("\\\"$")
'''
    new_exact_artist = '''            joiner.add("artist:\\\"")
            joiner.append(a)
            joiner.append("$\\\"")
'''
    if text.count(old_exact_artist) != 1:
        fail("Pinned ExHentai parser changed: artist exact query block not found exactly once")
    text = text.replace(old_exact_artist, new_exact_artist, 1)

    # Preserve the gallery title shown by E-Hentai/ExHentai. Upstream cleanup removes every [..]
    # group, which hides uploader/group prefixes and markers such as [AI Generated].
...
'''
    ...
    old_detail_titles = '''            title = title?.getElementById("gn")?.text()?.cleanupTitle() ?: manga.title,
            altTitles = setOfNotNull(title?.getElementById("gj")?.text()?.cleanupTitle()?.nullIfEmpty()),
'''
    new_detail_titles = '''            title = title?.getElementById("gn")?.text()?.trim()?.nullIfEmpty() ?: manga.title,
            altTitles = setOfNotNull(title?.getElementById("gj")?.text()?.trim()?.nullIfEmpty()),
'''
    if text.count(old_detail_titles) != 1:
