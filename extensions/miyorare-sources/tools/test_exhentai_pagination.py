...
class ExhentaiCursorRecoveryTest(unittest.TestCase):
    ...
    def test_generated_patch_has_exact_request_key_and_recovery_contract(self):
        ...
        self.assertIn('url.addEncodedQueryParameter("next", next.toString())', source)
        self.assertIn('if (controls.isNotEmpty())', source)
        self.assertIn('url.addQueryParameter("f_apply", "Apply Filter")', source)
        self.assertIn('url.addQueryParameter("advsearch", "1")', source)
        self.assertIn('"f_doujinshi"', source)
        self.assertIn('"f_misc"', source)
        self.assertIn('"f_sname"', source)
        self.assertIn('"f_stags"', source)
        self.assertIn('"f_sdesc"', source)
        self.assertIn('"f_storr"', source)
        self.assertIn('"f_sto"', source)
        self.assertIn('"f_sdt1"', source)
        self.assertIn('"f_sdt2"', source)
        self.assertIn('"f_sr"', source)
        self.assertIn('"f_srdd"', source)
        self.assertIn('"f_sp"', source)
        self.assertIn('"f_spf"', source)
        self.assertIn('"f_spt"', source)
