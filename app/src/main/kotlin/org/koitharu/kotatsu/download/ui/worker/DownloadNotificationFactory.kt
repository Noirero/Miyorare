...
class DownloadNotificationFactory @AssistedInject constructor(
	...
	private var privateStatusMangaId = 0L
	...
	init {
		createChannels(); builder.setOnlyAlertOnce(true); builder.setDefaults(0)
		...
		builder.setSilent(true); builder.setGroup(GROUP_ID); builder.setGroupAlertBehavior(NotificationCompat.GROUP_ALERT_CHILDREN)
	...
	suspend fun create(state: DownloadState?): Notification = mutex.withLock {
		...
		when {
			...
			state.isStopped -> { builder.setProgress(0, 0, false); builder.setContentText(context.getString(R.string.queued)); builder.setCategory(NotificationCompat.CATEGORY_PROGRESS); builder.setOngoing(true); builder.setSmallIcon(R.drawable.ic_stat_paused); builder.addAction(actionCancel) }
			state.isPaused -> { builder.setProgress(state.max, state.progress, false); val progressText = getProgressString(state.copy(eta = -1L, isStuck = false)); builder.setContentText(if (state.errorMessage != null) { if (redactPrivateDetails) context.getString(R.string.error) else if (progressText != null) context.getString(R.string.download_summary_pattern, progressText, state.errorMessage) else state.errorMessage } else progressText); builder.setCategory(NotificationCompat.CATEGORY_PROGRESS); builder.setOngoing(true); builder.setSmallIcon(R.drawable.ic_stat_paused); builder.addAction(actionCancel); if (state.errorMessage != null) { builder.addAction(actionRetry); builder.addAction(actionSkip) } else builder.addAction(actionResume) }
			state.isIndeterminate -> { builder.setProgress(1, 0, true); builder.setContentText(context.getString(if (redactPrivateDetails) R.string.manga_downloading_ else R.string.preparing_)); builder.setCategory(NotificationCompat.CATEGORY_PROGRESS); builder.setOngoing(true); builder.addAction(actionCancel); builder.addAction(actionPause) }
			state.error != null -> { val errorText = if (redactPrivateDetails) context.getString(R.string.error) else state.errorMessage; builder.setProgress(0, 0, false); builder.setSubText(context.getString(R.string.error)); builder.setContentText(errorText); builder.setAutoCancel(true); builder.setOngoing(false); builder.setCategory(NotificationCompat.CATEGORY_ERROR); builder.setShowWhen(true); builder.setWhen(System.currentTimeMillis()); builder.setStyle(NotificationCompat.BigTextStyle().bigText(errorText)) }
			else -> { builder.setProgress(state.max, state.progress, false); builder.setContentText(getProgressString(state)); builder.setCategory(NotificationCompat.CATEGORY_PROGRESS); builder.setOngoing(true); builder.addAction(actionCancel); builder.addAction(actionPause) }
		}
		if (state != null && redactPrivateDetails) { builder.setContentTitle(context.getString(R.string.manga_downloading_)); builder.setContentText(context.getString(if (state.localManga != null) R.string.download_complete else R.string.manga_downloading_)); builder.setLargeIcon(null as android.graphics.Bitmap?); builder.setContentIntent(queueIntent); builder.setStyle(null); builder.setSubText(null); builder.setVisibility(NotificationCompat.VISIBILITY_SECRET) }
		builder.build()
	}
	...
	@AssistedFactory interface Factory { fun create(uuid: UUID, isSilent: Boolean): DownloadNotificationFactory }
}
