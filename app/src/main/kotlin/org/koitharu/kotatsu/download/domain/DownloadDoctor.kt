...
object DownloadDoctor {
	...
	private fun hasJpegEnd(file: File): Boolean = RandomAccessFile(file, "r").use { input ->
		if (input.length() < 4L) return@use false
		input.seek(input.length() - 2L)
		input.readUnsignedByte() == 0xFF && input.readUnsignedByte() == 0xD9
	}

	private fun hasGifEnd(file: File): Boolean = RandomAccessFile(file, "r").use { input ->
		if (input.length() < 7L) return@use false
		input.seek(input.length() - 1L)
		input.readUnsignedByte() == 0x3B
	}

	private fun hasPngEnd(file: File): Boolean = RandomAccessFile(file, "r").use { input ->
		if (input.length() < PNG_END.size) return@use false
		input.seek(input.length() - PNG_END.size)
		val tail = ByteArray(PNG_END.size)
		input.readFully(tail)
		tail.contentEquals(PNG_END)
	}
