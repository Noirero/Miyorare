...
class NovelTranslationSecrets(context: Context) {
	...
	private fun encrypt(plainText: String): String {
		...
		// With Android Keystore + randomized encryption enabled, the IV for ENCRYPT_MODE must be
		// generated by the Keystore provider. Supplying our own IV is rejected on newer Android
		// versions (for example Android 16) with "Caller-provided IV not permitted".
		cipher.init(Cipher.ENCRYPT_MODE, secretKey())
		val iv = cipher.iv ?: error("Android Keystore did not provide an AES-GCM IV")
		check(iv.size == IV_BYTES) { "Unexpected AES-GCM IV length: ${iv.size}" }
